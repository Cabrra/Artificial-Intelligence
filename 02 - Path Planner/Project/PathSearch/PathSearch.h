...
namespace fullsail_ai { namespace algorithms {
	...
	public:
		PlannerNode(SearchNode* main = nullptr);
		void SetVertex(SearchNode * theVertex)
		{
			vertex = theVertex;
		}
		void SetParent(PlannerNode * theParent)
		{
			parent = theParent;
		}
		void SetGiven(double cost)
		{
			givenCost = cost;
		}
	...
	PlannerNode::PlannerNode(SearchNode* main)
	{
		SetVertex(main);
		parent = nullptr;
	}

	bool IsMyGreater(PlannerNode* const& lhs, PlannerNode* const& rhs)
	{
	...
	{
	private:
		unordered_map<pair<int, int>, SearchNode*, KeyHash, KeyEqual> pathNodes;
		SearchNode* goalTile, *firstTile;
		unordered_map<pair<int, int>, PlannerNode*, KeyHash, KeyEqual> visited;
		PriorityQueue<PlannerNode*> myPriority;
		bool isFinish;
		OwnTimer myTimer;
		TileMap* myTileMapCopy;
	...
	public:
		...
		double Estimate(pair<double, double> first, pair<double, double> second);
	};
